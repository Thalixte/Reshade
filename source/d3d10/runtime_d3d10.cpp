	...
	_app_state(device)
{
	...
	subscribe_to_load_config([this](const ini_file &config) {
		config.get("DX10_BUFFER_DETECTION", "DepthBufferRetrievalMode", _preserve_depth_buffer);
		...
		config.get("DX10_BUFFER_DETECTION", "PrimaryDepthBufferClearingNumber", cleared_primary_depth_buffer_index);
		config.get("DX10_BUFFER_DETECTION", "SecondaryDepthBufferClearingNumber", cleared_secondary_depth_buffer_index);
	});
	subscribe_to_save_config([this](ini_file &config) {
		config.set("DX10_BUFFER_DETECTION", "DepthBufferRetrievalMode", _preserve_depth_buffer);
		...
		config.set("DX10_BUFFER_DETECTION", "PrimaryDepthBufferClearingNumber", cleared_primary_depth_buffer_index);
		config.set("DX10_BUFFER_DETECTION", "SecondaryDepthBufferClearingNumber", cleared_secondary_depth_buffer_index);
	});
}
...
void reshade::d3d10::runtime_d3d10::draw_debug_menu()
{
	...
	if (ImGui::CollapsingHeader("Depth and Intermediate Buffers", ImGuiTreeNodeFlags_DefaultOpen))
	{
		...
		ImGui::Spacing();
		modified |= ImGui::Checkbox("Preserve depth buffer from being cleared", &_preserve_depth_buffer);
		...
		if (_preserve_depth_buffer)
		{
			...
			{
				cleared_primary_depth_buffer_index = cleared_secondary_depth_buffer_index = 0;
				modified = true;
			}
			...
			ImGui::Spacing();
			ImGui::TextUnformatted("Background Depth Buffers:");
			...
			for (const auto &it : _current_tracker->cleared_depth_textures())
			{
				...
				if (bool value = cleared_primary_depth_buffer_index == current_index; ImGui::Checkbox(label, &value))
				{
					...
					{
						modified = true;
						cleared_secondary_depth_buffer_index = 0;
					}
				}
			...
			ImGui::Spacing();
			ImGui::TextUnformatted("Foreground Depth Buffers:");

			current_index = 1;

			for (const auto &it : _current_tracker->cleared_depth_textures())
			{
				if (current_index > cleared_primary_depth_buffer_index)
				{
					char label[512] = "";
					sprintf_s(label, "%sSecondary %2u", (current_index == cleared_secondary_depth_buffer_index ? "> " : "  "), current_index);

					if (bool value = cleared_secondary_depth_buffer_index == current_index; ImGui::Checkbox(label, &value))
					{
						cleared_secondary_depth_buffer_index = value ? current_index : 0;
						modified = true;
					}
...
void reshade::d3d10::runtime_d3d10::detect_depth_source(draw_call_tracker &tracker)
{
	...
	if (_preserve_depth_buffer)
	{
		...
		ID3D10Texture2D *const best_match_texture = tracker.find_best_cleared_depth_buffer_texture(cleared_primary_depth_buffer_index, cleared_secondary_depth_buffer_index, false);
		_best_depthstencil_overwrite = nullptr;
